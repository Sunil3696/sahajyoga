...
import { HomeViewSystemSidebar } from "./system.sidebar.component";

export function HomeViewSystem() {
    let params = useParams();
    let [data, setData] = useState();
    console.log(params.id);

    useEffect(()=> {
        getItems(`systems/${params.id}`, false)
        .then((response)=> {
...
// console.log(data)
    ...
    ">
                    ...
                    <li className="font-semibold overflow-hidden text-base text-indigo-600 text-opacity-80">
                        {data?._id}
                    </li>
                </ul>
            </section>
            ...
            <section className=" mx-5 py-1 px-4 sm:px-6 md:px-8 md:py-5">
                <h1 className="md:text-3xl">{data?.name}</h1>
            </section>

            <section className="md:flex">
                <div className="md:w-3/4 w-full ">
                    <div className="my-2 mx-5">
                        <img src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.banner_image} alt="Picture" className="w-full h-auto" />
                    </div>
                    <div className="mx-5 my-2  text-justify">
                        <p dangerouslySetInnerHTML={{ __html: data?.top_desc }}>
                                ...
                                <div className="flex flex-wrap w-1/2">
                                    <div className="w-1/2 p-1 md:p-2">
                                        <img alt="gallery" className="block object-cover object-center w-full h-full rounded-lg"
                                            src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.images[0]} />
                                    </div>
                                    <div className="w-1/2 p-1 md:p-2">
                                        <img alt="gallery" className="block object-cover object-center w-full h-full rounded-lg"
                                            src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.images[1]} />
                                    </div>
                                    <div className="w-full p-1 md:p-2">
                                        <img alt="gallery" className="block object-cover object-center w-full h-full rounded-lg"
                                            src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.images[2]} />
                                    </div>
                                </div>
                                <div className="flex flex-wrap w-1/2">
                                    <div className="w-full p-1 md:p-2">
                                        <img alt="gallery" className="block object-cover object-center w-full h-full rounded-lg"
                                            src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.images[3]} />
                                    </div>
                                    <div className="w-1/2 p-1 md:p-2">
                                        <img alt="gallery" className="block object-cover object-center w-full h-full rounded-lg"
                                            src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.images[4]} />
                                    </div>
                                    <div className="w-1/2 p-1 md:p-2">
                                        <img alt="gallery" className="block object-cover object-center w-full h-full rounded-lg"
                                            src={data && process.env.REACT_APP_BASE_IMAGE_URL + "/" + data?.images[5]} />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mx-5 my-2  text-justify">
                        <p dangerouslySetInnerHTML={{ __html: data?.buttom_desc }}>
             ...
             <HomeViewSystemSidebar></HomeViewSystemSidebar>
            </section>
        </>
    )
}